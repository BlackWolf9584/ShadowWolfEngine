...
namespace SW 
{
	...
	bool SceneSerializer::Deserialize(const std::string& filepath)
	{
		...
		SW_CORE_INFO("Deserializing scene '{0}'", sceneName);
		...
		if (entities)
		{
			for (auto entity : entities)
			{
				...
				SW_CORE_INFO("Deserialized entity with ID = {0}, name = {1}", uuid, name);
				...
				if (transformComponent)
				{
					...
					SW_CORE_INFO("  Entity Transform:");
					SW_CORE_INFO("    Translation: {0}, {1}, {2}", translation.x, translation.y, translation.z);
					SW_CORE_INFO("    Rotation: {0}, {1}, {2}, {3}", rotation.w, rotation.x, rotation.y, rotation.z);
					SW_CORE_INFO("    Scale: {0}, {1}, {2}", scale.x, scale.y, scale.z);
				}
				...
				if (scriptComponent)
				{
					...
					SW_CORE_INFO("  Script Module: {0}", moduleName);
				...
				if (meshComponent)
				{
					...
					SW_CORE_INFO("  Mesh Asset Path: {0}", meshPath);
				}
				...
				if (cameraComponent)
				{
					...
					SW_CORE_INFO("  Primary Camera: {0}", component.Primary);
				}
