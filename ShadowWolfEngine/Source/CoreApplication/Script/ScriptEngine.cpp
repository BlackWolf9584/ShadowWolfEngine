...
namespace Wolf {
	...
	{
		SW_CORE_ASSERT(Handle, "Entity has not been instantiated!");
	...
	{
		SW_CORE_ASSERT(s_EntityInstanceMap.find(dst) != s_EntityInstanceMap.end());
		SW_CORE_ASSERT(s_EntityInstanceMap.find(src) != s_EntityInstanceMap.end());
	...
	{
		SW_CORE_ASSERT(s_EntityInstanceMap.find(sceneID) != s_EntityInstanceMap.end());
		auto& entityMap = s_EntityInstanceMap.at(sceneID);
		SW_CORE_ASSERT(entityMap.find(entityID) != entityMap.end());
	...
	void ScriptEngine::InitScriptEntity(Entity entity)
	{
		...
		{
			SW_CORE_ERROR("Entity references non-existent script module '{0}'", moduleName);
			return;
		}
	...
	void ScriptEngine::InstantiateEntityClass(Entity entity)
	{
		...
		EntityInstance& entityInstance = entityInstanceData.Instance;
		SW_CORE_ASSERT(entityInstance.ScriptClass);
	...
	{
		SW_CORE_ASSERT(s_EntityInstanceMap.find(sceneID) != s_EntityInstanceMap.end(), "Invalid scene ID!");
		auto& entityIDMap = s_EntityInstanceMap.at(sceneID);
		SW_CORE_ASSERT(entityIDMap.find(entityID) != entityIDMap.end(), "Invalid entity ID!");
	...
	{
		SW_CORE_ASSERT(m_EntityInstance->GetInstance());
