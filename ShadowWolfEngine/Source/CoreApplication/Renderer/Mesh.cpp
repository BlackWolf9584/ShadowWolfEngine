...
namespace SW
{
	...
	struct LogStream : public Assimp::LogStream
	{
		...
		{
			SW_CORE_ERROR("Assimp error: {0}", message);
		}
	};
		...
		: m_FilePath(filename)
	{
		...
		for (size_t m = 0; m < scene->mNumMeshes; m++)
		{
			...
			SW_CORE_ASSERT(mesh->HasPositions(), "Meshes require positions.");
			SW_CORE_ASSERT(mesh->HasNormals(), "Meshes require normals.");
	...
	{
		SW_CORE_ASSERT(pNodeAnim->mNumRotationKeys > 0);
	...
	{
		SW_CORE_ASSERT(pNodeAnim->mNumScalingKeys > 0);
